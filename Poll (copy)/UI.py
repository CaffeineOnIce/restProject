    ...
    with ui.left_drawer(value=True).classes("p-4").props("width=250 elevated"):
        ...
        def render_content(page_key):
            for key, btn in nav_buttons.items():
                if key == page_key:
                    btn.classes(remove="bg-transparent text-[var(--muted)]").classes(
                        add="bg-[#252A31] text-[var(--text)] font-medium"
                    )
                else:
            ...
            with content:
                if page_key == "overview":
                    ...
                    render_collect_card("temphum" if page_key == "th_range" else "gas")
